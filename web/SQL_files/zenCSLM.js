	...
	if (event.type=="mousedown") {
		...
		if (ZLM.eventBackup) {
			...
			if (s.initMouseEvent) { // all modern browsers
				...
				if (s.target && s.target.focus && s.target.id) {
...
ZLM.CSSPool = function(ssIdx) {
	try {		this.rules = [];		// DTB909 - Return empty array if neither rules exist
		this.ss=document.styleSheets[ssIdx];
		if (this.ss.cssRules) this.rules = this.ss.cssRules;		else if (this.ss.rules) this.rules = this.ss.rules;
	} catch (ex) {
		this.rules=[];
	}
}
...
ZLM.getRGBColor = function(node) {
	if (node.currentStyle) // IE Style
		...
		var clrNode = window.getComputedStyle(node,null);
		if (clrNode && clrNode.color) {
			clrStr=clrNode.color;
		}
		else {
			return(null);
		}
...
ZLM.getRGBBackgroundColor = function(node) {
	if (node.currentStyle) // IE Style
		...
		var clrNode = window.getComputedStyle(node,null);
		if (clrNode && clrNode.backgroundColor) {
			clrStr=clrNode.backgroundColor;
		}
		else {
			return(null);
		}
